// src/components/DriverFormModal.jsx
import React, { useMemo, useState } from "react";

const STATUS_OPTIONS = ["Registered", "Colorum", "Impounded", "Unavailable"];

export default function DriverFormModal({ show, mode, onClose, onSubmit }) {
  const title = useMemo(() => (mode === "COLORUM" ? "Colorum Form" : "Driver’s Form"), [mode]);
  ...
  const [form, setForm] = useState({
    ...
    chassis: "",
    plateNo: "",
    status: mode === "COLORUM" ? "Colorum" : "Registered",
    franchiseNo: mode === "COLORUM" ? "N/A" : "",
    photoFile: null,
  });
  ...
  function handleSubmit(e) {
    ...
    onSubmit({
      ...
      toda: form.toda.trim(),
      franchiseNo: mode === "COLORUM" ? "N/A" : form.franchiseNo.trim(),
      photoUrl,
      vehicle: {
        ...
        <div className="p-4">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3">
              ...
              <input className="form-control bg-light border-0 rounded-4" value={form.driverName} onChange={set("driverName")} />
            </div>

            <div className="mb-3">
              <div className="small text-muted mb-1">Operator’s Name</div>
              <input className="form-control bg-light border-0 rounded-4" value={form.operatorName} onChange={set("operatorName")} />
            </div>
            ...
            <div className="mb-3">
              ...
              <input className="form-control bg-light border-0 rounded-4" value={form.toda} onChange={set("toda")} />
            </div>

            <div className="mb-3">
              <div className="small text-muted mb-1">Motor</div>
              <input className="form-control bg-light border-0 rounded-4" value={form.motor} onChange={set("motor")} />
            </div>

            <div className="mb-3">
              <div className="small text-muted mb-1">Model / Make</div>
              <input className="form-control bg-light border-0 rounded-4" value={form.modelMake} onChange={set("modelMake")} />
            </div>

            <div className="mb-3">
              <div className="small text-muted mb-1">Engine</div>
              <input className="form-control bg-light border-0 rounded-4" value={form.engine} onChange={set("engine")} />
            </div>

            <div className="mb-3">
              <div className="small text-muted mb-1">Chassis</div>
              <input className="form-control bg-light border-0 rounded-4" value={form.chassis} onChange={set("chassis")} />
            </div>

            <div className="mb-3">
              <div className="small text-muted mb-1">Plate Number</div>
              <input className="form-control bg-light border-0 rounded-4" value={form.plateNo} onChange={set("plateNo")} />
            </div>

            <div className="mb-3">
              <div className="small text-muted mb-1">Status</div>
              <select className="form-select bg-light border-0 rounded-4" value={form.status} onChange={set("status")}>
